...
describes what users get. Pick a format with `-o`, or let mygene choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mygene <command> -o table   # aligned columns for reading
mygene <command> -o jsonl   # one JSON object per line, for piping
mygene <command> -o json    # a single JSON array
mygene <command> -o csv     # spreadsheet friendly
mygene <command> -o tsv     # tab-separated
mygene <command> -o url     # just the URL column
mygene <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mygene <command> --fields id,title,url
```
...
```bash
mygene <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mygene <command>            # a table, because this is a terminal
mygene <command> | wc -l    # JSONL, because this is a pipe
```
